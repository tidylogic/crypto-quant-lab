...
모든 프리셋은 [템플릿 인덱스](template-index.md)의 고정 계좌 한도를 준수합니다. 진입당 5%, 거래당 최대 세 번 진입 / 총 15%, 최대 두 개 포지션 / 총 30%, 5% 전략 드로다운 차단기 및 30% 일일 손실 차단기입니다. Long/short 수식은 정확히 대칭으로 적용해야 하며, 수수료, 스프레드, 슬리피지 및 펀딩은 평가 가정에 포함합니다.
...
- **금지:** 의사결정 시점에 이용할 수 없는 intrabar ATR을 사용하거나, 진입 후 `k`를 넓히거나, 무효화 후 추가 진입하지 않습니다.
...
- **입력 / 수식:** lookback `n`, buffer `b × ATR`; long stop = last confirmed swing low minus buffer, short stop = last confirmed swing high plus buffer.
...
- **유의사항:** 피벗 확인은 진입을 지연시키며 완료 캔들만으로 구현하지 않으면 look-ahead bias를 유발할 수 있습니다.

### STOP-CHANDELIER — ATR chandelier 추적 손절
...
- **트리거:** 진입 직후 초기 보호 손절을 제출하고, 완료 캔들 데이터로 유리한 방향으로만 chandelier 손절을 상향/하향 조정합니다.
- **금지:** 추적 수준을 느슨하게 하거나, 미래 고점/저점을 사용하거나, 무효화 수준 도달 후 포지션을 추가하지 않습니다.
- **주문 / 청산 행태:** 활성 손절에서 남은 포지션을 청산합니다. chandelier 추적이 유효해질 때까지 초기 보호 손절을 활성 상태로 유지합니다.
- **유의사항:** 이는 하나의 독립 Stop ID입니다. 초기 보호 손절과 chandelier 추적이 모두 여기 정의되어 있으므로 진입 리스크 정의에 다른 손절 규칙이 필요하지 않습니다. 횡보장에서 whipsaw에 취약합니다.
...
- **유의사항:** 부분 청산은 실현 승률을 높일 수 있지만 추세 포착을 낮출 수 있습니다. 승률만이 아니라 기대값과 Profit factor를 평가합니다.
...
- **주문 / 청산 행태:** reduce-only 부분 청산 뒤 이 프리셋의 추적 손절로 잔량을 관리합니다. 다른 선택 Stop ID와 짝지을 때는 이 프리셋의 유효한 잔량 추적이 시작될 때까지 그 Stop ID를 보호 손절로 유지합니다.
- **유의사항:** `TP-ATR-TRAIL`은 잔량의 추적 청산 규칙을 소유하므로, 역시 추적 청산을 소유하는 `STOP-CHANDELIER`와 결합할 수 없습니다. 다른 선택 Stop ID와 함께라면 잔량 추적이 유효해질 때까지 해당 손절을 보호로 유지합니다.

### TP-INDICATOR — 지표 정의 청산

- **목적 / 국면:** 인과적이고 관측 가능한 청산 신호가 있는 평균 회귀 또는 모멘텀 소진 설정.
- **입력 / 수식:** 명명한 지표, 시간대, 소스, 임계값/교차 조건 및 부분 또는 전체 청산 규칙. 예: 완료 캔들 RSI(14)가 사전 선언 수준 아래로 교차할 때 long을 청산합니다.
- **허용 범위:** 기간 5~50, 임계값 10~90, 부분 비율 25%~100%. 정확한 값은 사전 선언해야 하며 명명한 지표에 적합해야 합니다.
- **트리거:** 선언한 시간대에서 완전하고 인과적인 지표 조건이 발생할 때만 행동합니다.
- **금지:** 모호한 "momentum fades" 규칙, intrabar hindsight 또는 보호 손절을 우회하는 지표 청산을 사용하지 않습니다.
...
- **유의사항:** averaging down은 약한 진입 규칙을 숨기고 꼬리 손실을 확대할 수 있습니다. 명확한 기준선(`SCALE-NONE`)과 5% 이하의 승인 드로다운이 필요합니다.

### SCALE-PYRAMID — 확인 pyramiding
...
- **유의사항:** pyramiding은 추세 후기 리스크 근처의 노출을 높입니다. 실제 동시 총 배분이 30% 이하인지 확인합니다.
